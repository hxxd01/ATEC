"""Train Task-A student nav policy and optionally warm-start from BC checkpoint."""
...
parser = argparse.ArgumentParser(description="Train Task A student policy (BC warm-start + PPO).")
parser.add_argument("--task", type=str, default="ATEC-TaskA-B2Piper")
...
parser.add_argument("--vy_max", type=float, default=0.4)
parser.add_argument("--wz_max", type=float, default=0.4)
parser.add_argument("--init_x", type=float, default=-120.0, help="TaskA reset x position for robot root.")
parser.add_argument("--init_y", type=float, default=0.0, help="TaskA reset y position for robot root.")
parser.add_argument("--init_z", type=float, default=0.58, help="TaskA reset z position for robot root.")
...
parser.add_argument(
    ...
    type=float,
    default=0.0005,
    help="Constant time penalty added each low-level env step.",
)
...
from atec_rl_lab.train.nav.taska_student_env import TaskAStudentEnv
...
def _resolve_policy_path(path: str) -> str:
    """Resolve policy checkpoint path (supports repo-relative paths like demo/taska.pt)."""
    if not path:
    ...
    raise FileNotFoundError(
        f"ll_policy not found: {path}\n"
        f"Tried: {', '.join(sorted(seen))}\n"
        f"Example: --ll_policy demo/taska.pt"
    )
...
def main():
    ...
    if env_cfg.observations is not None:
        ...
        if getattr(env_cfg.observations, "extero", None) is not None:
            # Disable the whole extero group to avoid an empty concatenation group.
            env_cfg.observations.extero = None
    if getattr(env_cfg.scene, "lidar_sensor", None) is not None:
        env_cfg.scene.lidar_sensor = None
    if getattr(env_cfg, "terminations", None) is not None and getattr(env_cfg.terminations, "fall", None) is not None:
        # TaskA default sets this to -20.0, which lets episodes free-fall for too long.
        # Raise it for stable PPO rollouts.
        env_cfg.terminations.fall.params["minimum_height"] = 0.25
    if getattr(env_cfg.scene, "robot", None) is not None and getattr(env_cfg.scene.robot, "init_state", None) is not None:
        env_cfg.scene.robot.init_state.pos = (float(args_cli.init_x), float(args_cli.init_y), float(args_cli.init_z))
    ...
    agent_cfg.experiment_name = "taska_student_b2piper"
    ...
    print(
        f"[INFO] TaskA init pose: x={args_cli.init_x:.2f}, y={args_cli.init_y:.2f}, z={args_cli.init_z:.2f}",
        flush=True,
    )
    print("[INFO] Start TaskA student fine-tuning...", flush=True)
